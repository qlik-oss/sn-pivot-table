...
export default function supernova(env: Galaxy) {
  return {
    ...
    component() {
      ...
      const layoutService = createLayoutService(layout);
      ...
      useEffect(() => {
        if (dataModel.hasData && rect?.width && rect?.height && constraints && selections && viewService && layoutService) {
          console.debug('render', { layout: { ...layout }, selections, constraints, dataModel, rect, model, viewService, layoutService });
          render(element, {
            rect,
            constraints,
            dataModel,
